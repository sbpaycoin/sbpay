...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZSBPModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zsbpTracker->HasMintTx(hash))) {
            //zSBP stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSBP;
...
bool IsZSBPType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSBP:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSBP || type == TransactionRecord::MNReward) {
