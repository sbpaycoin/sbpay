...
#include "ui_zsbpcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zsbpTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsbpTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSBP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSBP seed used to mint this zSBP is not the same as currently hold in the wallet";
            else
