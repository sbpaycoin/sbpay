...
CzSBPWallet::CzSBPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsbp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSBPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSBPSeed_deprecated()) {
                    LogPrintf("%s: Updated zSBP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsbp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsbp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSBPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSBPCount(nCountLastUsed);
    else if (!walletdb.ReadZSBPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSBPWallet::Lock()
{
    seedMaster = 0;
}

void CzSBPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSBPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSBP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSBPWallet::LoadMintPoolFromDB()
{
...
void CzSBPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSBPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSBPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsbpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSBPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSBP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsbpTracker which also adds to database
    pwalletMain->zsbpTracker->Add(dMint, true);
...
void CzSBPWallet::SeedToZSBP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSBPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSBPWallet::GenerateDeterministicZSBP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSBPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSBPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSBPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
