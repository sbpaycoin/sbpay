...
CzSBPTracker::CzSBPTracker(std::string strWalletFile)
{
...
CzSBPTracker::~CzSBPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSBPTracker::Init()
{
...
bool CzSBPTracker::Archive(CMintMeta& meta)
{
...
bool CzSBPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSBPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSBPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSBPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSBPTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSBPTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSBPTracker::GetSerialHashes()
{
...
CAmount CzSBPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSBPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSBPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSBPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSBPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSBPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSBPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSBPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSBPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSBPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSBPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSBPWallet* zSBPWallet)
{
    bool iszSBPWalletInitialized = (NULL != zSBPWallet);
    CMintMeta meta;
    ...
    if (! iszSBPWalletInitialized)
        zSBPWallet = new CzSBPWallet(strWalletFile);
    meta.isSeedCorrect = zSBPWallet->CheckSeed(dMint);
    if (! iszSBPWalletInitialized)
        delete zSBPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSBPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSBPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSBPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSBPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSBPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSBPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSBPWallet* zSBPWallet = new CzSBPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSBPWallet);
        }
        delete zSBPWallet;
        LogPrint("zero", "%s: added %d dzsbp from DB\n", __func__, listDeterministicDB.size());
    }
