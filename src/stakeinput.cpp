...
// The zSBP block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSBP is a hash of the serial
...
{
    //Create an output returning the zSBP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSBPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSBP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSBP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSBPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSBP output", __func__);
...
{
    CzSBPTracker* zsbpTracker = pwallet->zsbpTracker.get();
    CMintMeta meta;
    if (!zsbpTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zsbpTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SBP stake is the outpoint
