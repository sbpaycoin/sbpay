...
public:
    ...
    bool WriteZSBPSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSBPSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSBPSeed_deprecated(uint256& seed);
    bool EraseZSBPSeed();
    bool EraseZSBPSeed_deprecated();

    bool WriteZSBPCount(const uint32_t& nCount);
    bool ReadZSBPCount(uint32_t& nCount);
